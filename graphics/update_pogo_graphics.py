...
def GetFilenamesPairs(gm_obj):
    ...
    if (
        "form" in gm_obj_s
        and isinstance(gm_obj_s["form"], str)
    ...
    if "tempEvoOverrides" in gm_obj_s:
        if id == 6 or id == 150:  # Charizard or Mewtwo
            filenames_pairs.append(
                ("pm" + str(id) + ".fMEGA_X.icon.png", name + "-megax.png")
            )
            filenames_pairs.append(
                ("pm" + str(id) + ".fMEGA_Y.icon.png", name + "-megay.png")
            )
        elif id == 382 or id == 383:  # Kyogre or Groudon
            filenames_pairs.append(
                ("pm" + str(id) + ".fPRIMAL.icon.png", name + "-primal.png")
            )
        else:
            filenames_pairs.append(
                ("pm" + str(id) + ".fMEGA.icon.png", name + "-mega.png")
            )
