...
def update_pokemon():
    ...
    df_pokemons_csv[pokemons_out_cols].to_csv("data/pokemon.csv", index=False)
...
def update_pokemon_parents(df_pokemons):
    ...
    # output to csv
    df_parents.to_csv("data/pokemon_parents.csv", index=False)
...
def update_pokemon_types(df_pokemons):
    ...
    df_pokemon_types.to_csv("data/pokemon_types.csv", index=False)
    return df_pokemon_types


def update_pokemon_types_effectiveness(df_pokemons, df_pokemon_types):
    df_type_effectiveness = pd.read_csv("data/type_effectiveness.csv")
    ...
    df.to_csv("data/pokemon_types_effectiveness.csv", index=False)
...
def update_moves():
    ...
    df_moves["effect"] = df_moves.apply(
        lambda row: parse_move_buffs_effect(row), axis=1
    )
    ...
    # output to csv
    df_moves_csv[moves_out_cols].to_csv("data/moves.csv", index=False)
...
def update_pokemon_moves(df_pokemons, df_moves, df_pokemon_types):
    ...
    df_pokemon_fast_moves["energy"] = df_pokemon_fast_moves["energy"].replace(
        0.0, np.nan
    )
    ...
    df_pokemon_fast_moves[fast_out_cols].to_csv(
        "data/pokemon_fast_moves.csv", index=False
    )
    ...
    df_pokemon_charged_moves[charged_out_cols].to_csv(
        "data/pokemon_charged_moves.csv", index=False
    )
    ...
    df_pokemon_combined_moves[combined_out_cols].to_csv(
        "data/pokemon_moves.csv", index=False
    )
...
def parse_move_buffs_effect(r):
    ...
    if r["buff_target"] == "both":
        ...
        buff_text = [
            f"{buff:+} {stat}"
            for buff, stat in zip(r.buffs, ["Atk", "Def"])
            if buff != 0
        ]
